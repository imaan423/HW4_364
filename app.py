...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/imunirHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs',db.Column('gif_id',db.Integer, db.ForeignKey('gif.id')),db.Column('searchterm_id',db.Integer, db.ForeignKey('searchterm.id')))
...
user_collection = db.Table('user_collections',db.Column('gif_id', db.Integer,db.ForeignKey('gif.id')),db.Column('Personalgifcollection_id',db.Integer,db.ForeignKey('Personalgifcollection.id')))
...
class User(UserMixin, db.Model):
    ...
    gif = db.relationship('PersonalGifCollection',backref='User')
...
def get_gifs_from_giphy(search_string):
    ...
    all_params = {'q':search_string,'api_key': api_key,'limit':5 }
    r = requests.get(baseurl, params = all_params)
...
def get_or_create_gif(title, url):
    ...
    if not gif:
        new = Gif(title = title, embedURL = url)
        db.session.add(new)
        db.session.commit()
        return new
    return gif
...
def get_or_create_search_term(term):
    ...
    search = SearchTerm.query.filter_by(term = term).count()
    if not search:
        gif_list = []
        l = get_gifs_from_giphy(term)
        for item in l:
            g = get_or_create_gif(item["title"], item["embed_url"])
            gif_list.append(g)
        n = SearchTerm(term=term, gifs=gif_list)
        db.session.add(n)
        db.session.commit()
    return db.session.query(SearchTerm).filter(SearchTerm.term==term).first()
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    get_user = User.query.filter_by(username=current_user.username).first()
    create_collection = db.session.query(PersonalGifCollection).filter_by(name=name, user_name=get_user.id).count()
    if not create_collection:
        new = PersonalGifCollection(name=name, user_name=get_user.id, gif=gif_list)
        db.session.add(new)
        db.session.commit()
        gif_list = []
        for item in gif_list:
            new.gif.append(item)
        return(new)
    return db.session.query(PersonalGifCollection).filter(PersonalGifCollection.name==name, PersonalGifCollection.user_name==current_user).first()
...
def create_collection():
    ...
    if form.validate_on_submit():
        ...
        l = []
        for item in gif_list:
            #item = Gif.query.filter_by(title=title).first()
            l.append(get_gif_by_id(item))
        print(l)
        get_or_create_collection(name=name, current_user=current_user,gif_list=l)
...
def collections():
    get_user = User.query.filter_by(username=current_user.username).first()
    name = PersonalGifCollection.query.filter_by(user_name = get_user.id).all()
    return render_template('collections.html',collections=name)
